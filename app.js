...
let sr;
function startSequence(){sr={level:1,seq:[],input:[],locked:true,score:0,responseTimeMs:0,responseInputs:0,responseStart:0};const grid=$('#sequence-grid');grid.innerHTML='';for(let i=0;i<9;i++){const b=document.createElement('button');b.className='sequence-tile';b.dataset.i=i;b.addEventListener('click',()=>sequencePress(i,b));grid.append(b)}show('sequence-screen');sequenceLevel()}
function sequenceLevel(){sr.input=[];sr.seq=[];sr.responseStart=0;const len=sr.level+2;while(sr.seq.length<len){const n=Math.floor(Math.random()*9);if(n!==sr.seq.at(-1))sr.seq.push(n)}$('#sequence-level').textContent=`${sr.level} / 9`;$('#sequence-message').textContent='Watch the sequence.';$('#sequence-progress').innerHTML=sr.seq.map(()=>'<span></span>').join('');sr.locked=true;let i=0;const play=()=>{if(i>=sr.seq.length){sr.locked=false;sr.responseStart=performance.now();$('#sequence-message').textContent='Repeat the sequence.';return}const tile=$$('.sequence-tile')[sr.seq[i]];tile.classList.add('active');setTimeout(()=>{tile.classList.remove('active');i++;setTimeout(play,160)},420-Math.min(sr.level*18,120))};setTimeout(play,600)}
function recordSequenceResponse(){if(!sr.responseStart)return;sr.responseTimeMs+=performance.now()-sr.responseStart;sr.responseInputs+=Math.max(1,sr.input.length);sr.responseStart=0}
function sequencePress(i,b){if(!sr||sr.locked)return;b.classList.add('active');setTimeout(()=>b.classList.remove('active'),120);const pos=sr.input.length;sr.input.push(i);if(i!==sr.seq[pos]){recordSequenceResponse();sr.locked=true;$('#sequence-message').textContent='Incorrect. Advancing with reduced score.';setTimeout(nextSequenceLevel,900);return}$('#sequence-progress').children[pos].classList.add('done');if(sr.input.length===sr.seq.length){recordSequenceResponse();sr.score+=sr.seq.length;sr.locked=true;$('#sequence-message').textContent='Sequence confirmed.';setTimeout(nextSequenceLevel,700)}}
function nextSequenceLevel(){if(sr.level===9){const avgPressMs=sr.responseInputs?sr.responseTimeMs/sr.responseInputs:1500;const completionRatio=sr.score/63;const speedFactor=Math.max(0,Math.min(1,(1500-avgPressMs)/950));const speedBonus=Number((4*speedFactor*completionRatio).toFixed(2));completeGame('sequence',{score:sr.score,responseTimeMs:Math.round(sr.responseTimeMs),responseInputs:sr.responseInputs,avgPressMs:Math.round(avgPressMs),speedBonus})}else{sr.level++;sequenceLevel()}}
